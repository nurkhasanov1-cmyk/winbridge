...
- CLI argument parsing rejects unknown, duplicate, missing-value, malformed relay URL, relay URLs with embedded credentials or canonical/case-variant `token` query values, malformed protocol identifier, blank, untrimmed, control-character, bidi/zero-width-control, or oversized display name, malformed permission, malformed pairing, blank, untrimmed, control-character, bidi/zero-width-control, or oversized token, zero or unsafe `--authorization-ttl-ms`, zero, unsafe, or prompt-disabled `--host-consent-timeout-ms`, blank, untrimmed, control-character, bidi/zero-width-control, or oversized audit log path, blank, untrimmed, or oversized lifecycle reason, non-`true`/`false` visible-session, host-consent-prompt, or host-control-prompt values, viewer prompt mode, host control prompt mode on viewers, host consent prompt combined with static approval/denial, host control prompt combined with host consent prompt, and malformed, host-mode, or `screen:view`-less viewer signal probe configuration before runtime start.
