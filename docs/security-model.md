...
- Host workflow audit records and local disconnect audit records can be persisted locally with `--audit-log` or `WINBRIDGE_AGENT_AUDIT_LOG_PATH`. Delayed and direct pause/resume audit failures are surfaced through sanitized runtime diagnostics and prevent the matching session-control, authorization-state, and lifecycle audit-event messages. Local disconnect audit failures are surfaced through sanitized runtime diagnostics but do not block immediate indicator deactivation or local WebSocket close.
