...
- Viewer status snapshots are read-only, viewer-only, and bounded to local lifecycle metadata such as state, host visibility, permission count, optional authorization id/status, optional relay-defined remote disconnect reason code after trusted host disconnect, and optional local inactive cause after explicit viewer leave. After trusted host disconnect, viewer status reports inactive local state, host visibility false, zero action-capable permissions, and the bounded disconnect reason code without rewriting the last authorization status. After managed local viewer leave, viewer status reports inactive local state, host visibility false, zero action-capable permissions, and a bounded `localInactiveCause` while omitting authorization id/status metadata from the left connection scope. The development CLI can print status once with `--viewer-status-after-ms <delay>`, which is viewer-only, accepts exact bounded millisecond delays, and does not require requested permissions. Reading or printing viewer status does not send protocol messages, emit workflow audit events, grant permissions, start signaling, reconnect peers, invoke host controls, or expose screen, input, clipboard, file-transfer, diagnostics, token, pairing, credential, peer-id, signal-payload, display-name, raw close reason text, or private-reason data.
