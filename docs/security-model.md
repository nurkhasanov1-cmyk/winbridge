...
- Viewer status snapshots are read-only, viewer-only, and bounded to local lifecycle metadata such as state, host visibility, permission count, and optional authorization id/status. The development CLI can print them once with `--viewer-status-after-ms <delay>`, which is viewer-only, accepts exact bounded millisecond delays, and does not require requested permissions. Reading or printing viewer status does not send protocol messages, emit workflow audit events, grant permissions, start signaling, invoke host controls, or expose screen, input, clipboard, file-transfer, diagnostics, token, pairing, credential, peer-id, signal-payload, display-name, or private-reason data.
