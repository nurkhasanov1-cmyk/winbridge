...
- CLI parsing rejects unknown, duplicate, missing-value, malformed protocol identifier, malformed permission, malformed pairing, malformed lifecycle reason, and non-`true`/`false` `--visible-session` values before starting the runtime.
